...
int rknn_lite::interf()
{
    ...
    // std::cout << "sizeof outputs[0] " << sizeof(outputs[0].buf) << std::endl;
    // 总之就是绘图部分
    // post process
    ...
    for (int i = 0; i < detect_result_group.count; i++)
    {
        ...
        if (!strncmp(det_result->name,"person",6))
        {
          ...
          Eigen::Vector3d distance = DE.distance(kuang);
          sprintf(text, "%s %.1f%% d=%.2f m", det_result->name, det_result->prop * 100, distance[0]);
